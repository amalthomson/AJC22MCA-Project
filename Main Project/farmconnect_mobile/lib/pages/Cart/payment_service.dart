...
import 'package:farmconnect/pages/Cart/order_confirmation.dart';
...
class PaymentService {
  ...
  static Future<void> _storePaymentDetails(BuildContext context, String paymentId) async {
    ...
    try {
      ...
      DocumentSnapshot userAddressSnapshot =
      ...
      for (var item in cartProvider.cartItems) {
        // Fetch product details from Firestore to get the userId
        DocumentSnapshot productSnapshot =
        await FirebaseFirestore.instance.collection('products').doc(item['productId']).get();
        Map<String, dynamic> productData = productSnapshot.data() as Map<String, dynamic>;

        // Parse productPrice as double
        double productPrice = double.tryParse(productData['productPrice'] ?? '0.0') ?? 0.0;
        double totalPrice = productPrice * (item['quantity'] ?? 1);

        // Include the userId associated with the product
        ...
        // Update the userId associated with the product in the database
